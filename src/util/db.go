...
func init() {
	...
	fmt.Println("init db")
	// 创建数据库
	// stmt, _ := db.Prepare("CREATE DATABASE IF NOT EXISTS ?")
	// _, err = stmt.Exec(conf.MySQL.DBName)
	_, err = db.Exec("CREATE DATABASE IF NOT EXISTS member")
	if err != nil {
		fmt.Println(err)
		return
	}
	_, err = db.Exec("use `" + conf.MySQL.DBName + "`")
	_, err = db.Exec("CREATE TABLE IF NOT EXISTS `user` (`id` INT UNSIGNED AUTO_INCREMENT, `name` VARCHAR(32) NOT NULL, `nickname` VARCHAR(32) NOT NULL, `email` VARCHAR(32) NOT NULL, `phone_number` VARCHAR(32) NOT NULL, `bcrypt_pw` VARCHAR(64) NOT NULL, `group` VARCHAR(32) NOT NULL, `status` INT NOT NULL, `create_date` DATE NOT NULL, PRIMARY KEY(id))ENGINE=InnoDB DEFAULT CHARSET=utf8")
	if err != nil {
		fmt.Println(err)
		return
	}
